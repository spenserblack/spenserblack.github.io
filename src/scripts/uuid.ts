...
export function uuidV5(value: string, raw: true): number[];
export function uuidV5(value: string, raw?: boolean): string | number[] {
  if (raw) {
    const buffer = new Uint8Array(16);
    return Array.from(uuidV5Impl(value, namespace, buffer));
  }
